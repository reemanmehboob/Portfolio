...
function Navbar() {
  ...
  return (
    <div className='max-w-screen-2xl container mx-auto px-4 h-16 md:px-20 shadow-md fixed-top-0 left-0 right-0 buttom-0 z-50 bg-white'>
     <div className='flex justify-between items-center h-30'> 
        <div className='flex space-x-2'>
            <img src={pic} className='h-12 w-12 rounded-full' alt=''/>
            <h1 className='font-semibold text-x1 cursor-pointer'>
                Reema<span className='text-green-500 text-2xl'>N</span>
     ...
     {/*mobile navbar*/}
      {menu &&(
         ...
         {
                navItems.map(({id,text})=>(
                  <li className='hover:scale-105 space-y-3 text-xl font-semibold duration-200 cursor-pointer' key={id}><Link onClick={()=>setMenu(!menu)} to={text} smooth={true} duration={500} offset={-70} activeClass='active'>{text}</Link> </li>
                ))
               }
